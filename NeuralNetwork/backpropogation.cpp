...
void NeuralNetwork::backPropagation() {
  ...
  deltaWeights  = new Matrix(
                    ...
                    zActivatedVals->getCols(),
                    false
                  );

  Math::multiplyMatrix(gradientsTransposed, zActivatedVals, deltaWeights);
  ...
  for(int i = (indexOutputLayer - 1); i > 0; i--) {
    ...
    gradients   = new Matrix(
                    pGradients->getRows(),
                    transposedPWeights->getCols(),
                    false
                  );

    Math::multiplyMatrix(pGradients, transposedPWeights, gradients);

    hiddenDerived       = this->layers.at(i)->matrixifyDerivedVals();

    for(int colCounter = 0; colCounter < hiddenDerived->getRows(); colCounter++) {
    ...
    Math::multiplyMatrix(transposedHidden, gradients, deltaWeights);
